...
When the new value depends on the current value, functional updates are often safer.
...
This is especially useful when multiple updates can happen close together.
...
Use the one that makes the state change easiest to understand.
